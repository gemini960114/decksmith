...
import { MODEL_CONFIG, PDF_CONFIG, APP_CONFIG, PPTX_CONFIG } from './constants';
...
function App() {
  ...
  const [globalScale, setGlobalScale] = useState(PDF_CONFIG.DEFAULT_GLOBAL_SCALE); 
  ...
  const [globalEnableVerification, setGlobalEnableVerification] = useState(false); 
  const [exportLayout, setExportLayout] = useState(PPTX_CONFIG.DEFAULT_LAYOUT);
  
  // Modal State - Store ID instead of object to prevent stale references
  const [editingPageId, setEditingPageId] = useState<number | null>(null);
  const editingPage = useMemo(() => pages.find(p => p.id === editingPageId) || null, [pages, editingPageId]);
  ...
  const updatePageStatus = useCallback((pageId: number, status: PageStatus) => {
    setPages(prev => prev.map(p => p.id === pageId ? { ...p, status } : p));
  }, []);

  const updatePageInState = useCallback((updatedPage: PdfPage) => {
  ...
  useEffect(() => {
    ...
    const autoSave = async () => {
        const updatedJob: ProjectJob = {
            ...
            pages: pages.map(stripImagesFromPage)
        };
  ...
  const handleFileSelect = async (file: File) => {
    setIsParsing(true);
    try {
      const jobId = createNewJob(file.name);
      const parsedPages = await parsePdf(
        file, 
        globalScale, 
        globalOcrModel, 
        globalCleaningModel, 
        globalEnableVerification
      );
  ...
  const processPageLogic = async (page: PdfPage, forceSkipOcr: boolean = false) => {
      ...
      try {
          const [ocrResult, cleanedImage] = await Promise.all([
              (!forceSkipOcr || processedPage.textBlocks.length === 0)
                ? extractTextFromImage(processedPage.originalDataUrl, ocrModel, apiKey, 'detailed')
                : Promise.resolve(processedPage.textBlocks),
              
              removeTextFromImage(processedPage.originalDataUrl, cleaningModel, apiKey)
          ]);

          processedPage.textBlocks = ocrResult;
          processedPage.cleanedDataUrl = cleanedImage;
          updatePageInState(processedPage);
      } catch (e) {
          console.error(`Parallel processing failed for page ${processedPage.id}`, e);
          updatePageStatus(processedPage.id, PageStatus.ERROR);
          return false;
      }

      if (!processedPage.cleanedDataUrl) {
          updatePageStatus(processedPage.id, PageStatus.ERROR);
          return false;
      }
      ...
      if (enableVerify) {
          ...
          let residueBlocks = [];
          try {
              residueBlocks = await extractTextFromImage(processedPage.cleanedDataUrl, ocrModel, apiKey, 'simple');
          } catch (e) {
              console.warn(`Verification OCR failed for page ${processedPage.id}.`, e);
          }
          ...
          if (residueBlocks.length > 0) {
              ...
              try {
                ...
                if (refinedImage) {
                  ...
                  console.error(`Verification cleaning failed for page ${processedPage.id}`, e);
              }
          }
      }
  ...
  const startProcessing = useCallback(async () => {
    ...
    // Filter active processing target list once
    const targets = pages.filter(p => p.selected && p.status !== PageStatus.DONE);
    for (const page of targets) {
        await processPageLogic(page, false); 
        await new Promise(r => setTimeout(r, 200)); 
    }

    setIsProcessing(false);
  }, [pages, isProcessing, processedCount, selectedPagesCount, globalOcrModel, globalCleaningModel, apiKey, globalEnableVerification]);

  const handleReprocessPage = async (pageId: number, newScale: number, newOcrModel: string, newCleaningModel: string, updatedBlocks?: TextBlock[], newEnableVerification?: boolean) => {
  ...
  /**
   * New: Updates only metadata (text block inclusion) without reprocessing
   */
  const handleUpdateMetadata = useCallback((pageId: number, updatedBlocks: TextBlock[]) => {
      setPages(prev => prev.map(p => {
          if (p.id === pageId) {
              return { ...p, textBlocks: updatedBlocks };
          }
          return p;
      }));
  ...
  const handleExport = async () => {
    ...
    await generatePptx(selectedPages, `DeckSmith_${jobName}_${Date.now()}`, exportLayout);
  };
              ...
              <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-6">
                        <div className="text-xs font-black text-slate-300 uppercase tracking-wider border-b border-slate-100 pb-1">Rendering & Workflow</div>
                        
                        <div className="space-y-2">
                            <div className="flex justify-between items-center">
                                <label className="text-xs font-bold text-slate-500">Render Resolution (Max Dimension)</label>
                                <span className="text-xs font-mono bg-slate-100 px-2 py-0.5 rounded text-slate-700">{globalScale}px</span>
                            </div>
                            <div className="grid grid-cols-5 gap-1.5">
                                {PDF_CONFIG.GLOBAL_SCALE_OPTIONS.map(val => (
                                    <button
                                        key={val}
                                        onClick={() => setGlobalScale(val)}
                                        className={`py-2 text-[10px] font-black rounded-lg border transition-all ${globalScale === val ? 'bg-brand-600 border-brand-600 text-white' : 'bg-slate-50 border-slate-200 text-slate-500 hover:border-brand-300'}`}
                                    >
                                        {val}
                                    </button>
                                ))}
                        ...
                        <div className="pt-4 mt-2 border-t border-slate-50">
                            ...
                            <p className="text-[10px] text-slate-400 mt-1 pl-1">
                                High accuracy mode. Runs a second pass to ensure 100% background reconstruction.
                            </p>
                        </div>
                      </div>

                      <div className="space-y-4">
                        <div className="text-xs font-black text-slate-300 uppercase tracking-wider border-b border-slate-100 pb-1">AI Models</div>
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-slate-500 block">Cleaning Model (Independent Redraw)</label>
                            <select 
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-3xl shadow-sm border border-slate-100 sticky top-20 z-40">
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex items-center gap-4">
                             ...
                             <div className="flex items-center gap-2">
                                <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">Aspect Ratio:</span>
                                <select 
                                    value={exportLayout}
                                    onChange={(e) => setExportLayout(e.target.value)}
                                    className="text-[10px] font-bold border border-slate-200 rounded px-2 py-1 bg-slate-50 focus:outline-none focus:ring-1 focus:ring-brand-500"
                                >
                                    {PPTX_CONFIG.LAYOUTS.map(l => <option key={l.value} value={l.value}>{l.label}</option>)}
                                </select>
                             </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                    {pages.map(page => (
                        <PageCard 
                          key={page.id} 
                          page={page} 
                          onEdit={(p) => setEditingPageId(p.id)} 
